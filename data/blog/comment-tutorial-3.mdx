---
title: 'Build comments section with Next.js, SWR and Supabase - Part 3. Update, Delete and Reply'
...
_This is a 3rd part of the `Build comments section with Next.js, SWR and Supabase` series.
...
Try editing any comment, and then refresh. If succeeded then you'll see your comment edited.
...
- Our `confirmEdit` function always passes `updated_at: new Date()` so even though we've mistakenly confirm the edit without changing,
...
...
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const { data, error } = await supabase.from("comments").insert({
      ...
      payload: comment,
      reply_of: replyOf, // This one added
    });
    if (!error && data) {
      window.alert("Hooray!");
    } else {
